...
document.addEventListener("DOMContentLoaded", function () {
	...
	const arm_sliders = Array.from(
		document.getElementsByClassName("armSlider")
	);
	const light_button = document.getElementById("lightButton");
	const sound_buttons = Array.from(
		document.getElementsByClassName("soundButton")
	);

	var get_automatic_mode_service = new ROSLIB.Service({
		ros: ros,
		name: "/get_automatic_mode",
		serviceType: "wall_e_msg_srv/GetAutomaticMode",
	});
	...
	document.addEventListener("mousemove", function (event) {
		...
		event.preventDefault(); // Prevent page scrolling when dragging
...
function play_sound(sound_number) {
	...
	progress_bar.style.transition =
		"width " + transition_duration + "ms linear";
...
function set_volume(volume) {
	...
	set_volume_service.callService(request, function (result) {
		if (result.success) console.log("WALL-E modified volume with success.");
		else console.log("Error while modifying the volume.");
	});
}

var set_light_intensity_service = new ROSLIB.Service({
	ros: ros,
	name: "/set_light_intensity",
	serviceType: "wall_e_msg_srv/SetLightIntensity",
});
...
function switch_light() {
	...
	set_light_intensity_service.callService(request, function (result) {
		if (result.success)
...
function switch_automatic_mode() {
	...
	camera_light_on = false;

	var request = new ROSLIB.ServiceRequest({
		light_id: "camera_light",
		intensity_pct: parseInt(0, 10),
	});

	set_light_intensity_service.callService(request, function (result) {
		if (result.success)
			console.log("WALL-E switched led light on/off with success.");
		else console.log("Error while switching camera light on/off.");
	});
	...
	switch_automatic_mode_service.callService(request, function (result) {
		if (result.success)
			console.log("WALL-E switched automatic mode with success.");
