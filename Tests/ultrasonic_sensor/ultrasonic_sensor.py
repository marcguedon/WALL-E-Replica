...
def getDistance(sensor):
    ...
    if(sensor == 'left'):
        ...
        while GPIO.input(BCM_LEFT_ECHO_PIN) == 0:
            startTime = time.time()

        while GPIO.input(BCM_LEFT_ECHO_PIN) == 1:
            stopTime = time.time()
    ...
    if(sensor == 'right'):
        ...
        while GPIO.input(BCM_RIGHT_ECHO_PIN) == 0:
            startTime = time.time()

        while GPIO.input(BCM_RIGHT_ECHO_PIN) == 1:
            stopTime = time.time()
...
while (1):
    ...
    print('Left sensor: ' + str(leftDistance) + 'cm')
    print('Right sensor: ' + str(rightDistance) + 'cm\n')
